...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        // Use skip()
        long count = states.stream().count()-5;
        List<StateInfo> lastFiveStates =  states.stream().skip(count).collect(toList());
        ...
        List<Integer> collect = firstFiveNumbers.stream().limit(5).collect(toList());
        testResults.put("B3", StatesAndCapitalsCheck.basic3(collect));
       ...
       Long count1 = lastFiveNumbers.stream().count()-5;
        List<Integer> lastFiveNum =  lastFiveNumbers.stream().skip(count1).limit(5).collect(toList());
        testResults.put("B4", StatesAndCapitalsCheck.basic4(lastFiveNum));
        ...
        // Can use filter()
//***********************************************************************************************************
        StateInfo cardinalState = states.stream().filter(stateInfo -> stateInfo.getStateBird().equals("cardinal")).findFirst().orElseThrow();
        ...
        Boolean doAllStatesHaveAnAnthem = states.stream().allMatch(stateInfo -> Boolean.parseBoolean(stateInfo.getStateAnthem()));
...
//        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(stateInfo -> stateInfo.getStateMotto());
        ...
        Double averageYearlyPrecipitationAcrossStateCapitals = states.stream().collect(averagingDouble(capital -> capital.getCapital().getAverageYearlyPrecipitationInInches()));
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().mapToInt(captial->captial.getCapital().getAverageYearlyPrecipitationInInches()).sum();
        ...
        Map<String, Long> numberOfStatesByTimeZone = states.stream().
                collect(Collectors.groupingBy(state -> state.getTimeZones().toString() , counting() ));
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream().
                collect(Collectors.groupingBy(state -> state.getCapital().getTimeZone().toString() , counting() ));
        ...
        List<String> allDistinctStateBirds = states.stream().map(stateInfo -> stateInfo.getStateBird()).distinct().collect(toList());
